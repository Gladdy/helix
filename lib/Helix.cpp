...
class HelixImpl : public PassInfoMixin<HelixImpl>
{
public:
  HelixImpl()
  {
                        ...
                        LoopStandardAnalysisResults &AR, LPMUpdater &U)
  {
    ...
    // OuterAnalysisManagerProxy<FunctionA, Loop>
    // DominatorTree &DT = getAnalysis<DominatorTreeWrapperPass>().getDomTree();
    const PostDominatorTree *PDT = AM.getResult<FunctionAnalysisManagerLoopProxy>(L, AR).getManager().getCachedResult<PostDominatorTreeAnalysis>(*f);
...
private:
  ...
  {
    StructType *thread_data_t = mod->getTypeByName("struct.ThreadBufferBase");
  ...
  {
    OriginalLoopDescription(LLVMContext &ctx, Loop *L, const PostDominatorTree *PDT)
    {
  ...
  {
    Function *worker = cast<Function>(mod->getOrInsertFunction(name, ftype).getCallee());
    ...
    auto wait_signal_type = FunctionType::get(Type::getVoidTy(ctx),
                                              {thread_data_t->getPointerTo(), Type::getInt8Ty(ctx)},
                                              false);
    Function *thread_wait = cast<Function>(mod->getOrInsertFunction("thread_wait", wait_signal_type).getCallee());
...

    {
      ...
      IRBuilder<> b(hb.end);
      auto stop_sig = FunctionType::get(Type::getVoidTy(ctx),
                        ...
                        StructType *thread_data_t, Type *void_type)
  {
...

    {
      ...
      auto sched_func = FunctionType::get(
          Type::getVoidTy(ctx),
          {worker->getType(), void_type, Type::getInt32Ty(ctx)}, false);
      Function *scheduler = cast<Function>(mod->getOrInsertFunction(
                                                  "schedule_helix", sched_func)
                                               .getCallee());
...
llvm::PassPluginLibraryInfo getHelixPluginInfo()
{
  return {LLVM_PLUGIN_API_VERSION, "loop-helix", LLVM_VERSION_STRING,
          [](PassBuilder &PB) {
            PB.registerPipelineParsingCallback(
                [](StringRef Name, LoopPassManager &FPM, ArrayRef<PassBuilder::PipelineElement>) {
                  if (Name == "loop-helix")
                  {
                    FPM.addPass(HelixImpl());
                    return true;
                  }
                  return false;
                });
